...
export function AnnualIncomeStep({ data, updateData, onNext, onBack }: StepProps) {
  const income = data.annualIncome ?? 80000
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateData({ annualIncome: Math.min(Math.max(value, 0), MAX_INCOME) })
  }

  const clearIncome = () => {
    updateData({ annualIncome: MIN_INCOME })
  }
  ...
  const sliderPercentage = ((income - MIN_INCOME) / (MAX_INCOME - MIN_INCOME)) * 100

  return (
    <div className="w-full max-w-lg mx-auto">
      <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-8 text-center">
        What is the gross annual income of the borrower or borrowers?
      </h2>
